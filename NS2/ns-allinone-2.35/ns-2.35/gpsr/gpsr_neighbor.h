...
private:
  ...
  {
    return (getdis(my_x_, my_y_, x, y) < safedis) ? true:false;  
  }
...
public:
  ...
  void newNB(nsaddr_t, double, double, double, double); 
  void NB_test(nsaddr_t);//create by myself
  //decide the next hop based on the destination(x, y)
  nsaddr_t gf_nexthop(double, double); //greedy forwarding mode
  nsaddr_t gf_nexthop1(double, double,nsaddr_t ,double , double );
  double gf_nexthop2(double, double,nsaddr_t ,double , double ,double ,double);
  double gf_nexthop3(double , double );
  ...
  void dump();
  void dump(struct gpsr_neighbor *);

  void clear();
   //delete all the entries time out
  void delalltimeout();
  void SetSafetyDis ( double dis )
  {
    safedis = dis;
    
  }
  void SetHelloPeriod ( double t)
    {
      hello_period_ = t;
      
    }
  
  bool getnb(nsaddr_t, struct gpsr_neighbor &);
};
